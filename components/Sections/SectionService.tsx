...
const SectionService = ({services}) => {
	...
	const serv = services.data.map (item => ({
		category: item.attributes.service_category.data.attributes.title,
		title: item.attributes.title,
		price: item.attributes.price
	}))
	function sortByCat(arr: { price: number; link: string; title: string; category: string }[]) {
		let sortedData:any = {};
		arr.forEach(function (item: {
			...
			if(item.category in sortedData) {
				sortedData[item.category].items.push({
					title: item.title,
					price: item.price
				})
			} else {
				sortedData[item.category] = {
	...
	return (
		<Section
			...
			type={'section-service section-bg-top'}
			content={result.map((item, index) => <ServicePanel key={index}
				// @ts-ignore
			   items={item.items} state={state} index={index}/>)}
			tabbar={
				<ul className={'section__service-tabs'}>
					{result.map((item, index) => {
						return (<li key={index}><a className={`button button-link ${state === index ? 'active' : ''}`} onClick={() => setState(index)}
							// @ts-ignore
						>{item.category}</a> </li>)
					})}
				</ul>
